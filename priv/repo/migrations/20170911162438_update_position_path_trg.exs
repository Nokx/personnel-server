defmodule PersonnelServer.Repo.Migrations.UpdatePositionPathTrg do
  use Ecto.Migration
    ...
    CREATE OR REPLACE FUNCTION update_position_path() RETURNS TRIGGER AS $$
        ...
        BEGIN
            ...
            IF NEW.parent_id IS NULL THEN
                SELECT id from positions WHERE parent_id IS NULL INTO root_id;
                IF root_id IS NOT NULL THEN
                   IF (TG_OP = 'UPDATE' AND root_id != OLD.id) OR TG_OP = 'INSERT' THEN
                      RAISE EXCEPTION 'Возможно создать только одну корневую должность';
                   END IF;
                ELSE	
            ...
            ELSEIF TG_OP = 'INSERT' OR OLD.parent_id IS NULL OR OLD.parent_id != NEW.parent_id THEN
                ...
                ELSE
                    ...
                    -- Нужно обновить подразделение у должности, которую непосредственно изменяем,
                    -- так как её подразделение, учитывая что она не соответствует главе подразделения,
                    -- зависит от подразделения родительской должности 
                    NEW.department_id = dep_id;
                END IF;
                END IF;
            END IF;
